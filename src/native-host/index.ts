...
async function main(): Promise<void> {
  if (process.argv.includes("--self-test")) {
    process.stdout.write(`${JSON.stringify({ ok: true, protocol: RuntimeProtocolVersion, version: ASR_VERSION })}\n`);
    return;
  }

  const runtimeHome = defaultRuntimeHome();
  if (process.argv.includes("--asr-daemon")) {
    const daemon = new RuntimeDaemon({ runtimeHome, runtimeVersion: ASR_VERSION });
    await daemon.start();
